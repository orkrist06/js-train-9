...
function getPositiveArrayValues(arr) {
  ...
  let tempArray = Array.from(iterator);
  const positiveArrayValues = tempArray.filter((element) => element > 0);
...
function sumAndPrint(arr) {
  ...
  let sum = arr.reduce((result, element) => {
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let tempArray = arr.flat().map((element) => element ** 2);
  let filteredSquaredArray = tempArray.filter((element) => element > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let tempArray = arr.slice(-n);
  let sum = tempArray.reduceRight((result, element) => {
    return (result += element);
  }, 0);
...
function wordLengths(arr) {
  ...
  let lengthsArray = arr.map((element) => {
...
function expandArrayByN(arr, n) {
  ...
  let expandArray = arr.flatMap((element) => {
    let tempArray = [];
    for (let i = 0; i < n; i++) {
...
function calculateTotalPrice(arr) {
  ...
  let totalPrice = arr.reduce((result, { price }) => {
    return (result += price);
  }, 0);
